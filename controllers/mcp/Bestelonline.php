...
class Bestelonline extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model($this->template."/Mbestelonline");
	...
	function index() {
		...
		if($this->input->post()){
			if($_FILES['banner']['name']){
			  	...
			  	$filename = 'bestelonline_banner_'.time().'.'.end(explode(".",$_FILES['banner']['name']));
			  	...
			  	else
			  	{
					...
					$this->Mbestelonline->add($arr);
			  	}
	
				redirect(base_url().'mcp/bestelonline');
			}	
		}
		
		$data ['tempUrl'] = $this->tempUrl;
		$data ['content'] = $this->Mbestelonline->get();
		
		$data ['header'] = $this->template . '/header';
		$data ['main'] = $this->template . '/bestelonline';
	...
	function change_status(){
		...
		if( $this->input->post("banner_id") ){
			if( $this->Mbestelonline->update( $this->input->post("banner_id"), $this->input->post("status") ) ){
	...
	function delete(){
		$response = array( "error" => 1, "message" => _("Error occured. Please try again."));
		if( $this->input->post("banner_id") ){
			if( $this->Mbestelonline->delete( $this->input->post("banner_id") ) ){
