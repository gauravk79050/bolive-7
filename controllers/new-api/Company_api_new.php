 ...
 */
class Company_api_new extends REST_Controller
{
	/**
	 * Defining variable
	 */
	...
	*/
	...
	{
		/*$fp = fopen(APPPATH . "/controllers/new-api/log-init.txt","a");
		fwrite($fp,"data: ".json_encode($_SERVER)."\n");
		fclose($fp);*/
		...
		// Commenting this authentication so as to direct send back company id (27-09-2016)
		// $this->company_id = $this->api_verification->authenticate_login($verification_data);  

		// THis is the temporary code to let response company id quickly
		$company_id = $this->db->get_where('api', array('api_id' => $this->get('api_id')))->result_array();
		$this->company_id = $company_id['0']['company_id'];
		...
		{		
			...
			$result_arr = $this->db->get_where('api',array('domain'=> $host))->result();
			if( !empty($result_arr) && $result_arr[0]->company_id != '' )
			{
				$this->company_id = $result_arr[0]->company_id;
	...
	function company_settings_post()
	{
		...
		if($company_id)
		{
			$this->db->select('
					...
					general_settings.show_searchbox,
					general_settings.msg_front_txt_color,
					general_settings.shop_view
					');
			...
			$this->db->join('themes', 'themes.id = general_settings.theme_id','left');
	...
	function get_shop_content_all_post(){
		...
		// Fetching Opening hours
		$this->benchmark->mark('gs_start');
		$general_settings = $this->get_general_settings($company_id);
		$this->benchmark->mark('gs_end');
		
		// Fetching Categories
		$this->benchmark->mark('cat_start');
		$categories = $this->get_categories($company_id);
		$this->benchmark->mark('cat_end');
		
		// Fetching Subcategories
		$this->benchmark->mark('subcat_start');
		$subcategories = $this->get_subcategories($company_id);
		$this->benchmark->mark('subcat_end');
		...
		// Fetching Delivery settings
		$this->benchmark->mark('ds_start');
		$delivery_settings = $this->get_delivery_settings($company_id);
		$this->benchmark->mark('ds_end');

		// Fetching Delivery areas
		$this->benchmark->mark('da_start');
		$delivery_areas = $this->get_delivery_areas($company_id);
		$this->benchmark->mark('da_end');
		
		// Fetching International Delivery settings
		$this->benchmark->mark('is_start');
		$international = array();
		$company_countries = array();
		$company_countries_int = array();
		$all_countries = array();
		...
		$this->benchmark->mark('is_end');
		
		// Fetching Opening hours
		$this->benchmark->mark('oh_start');
		$opening_hours = $this->get_opening_hours($company_id);
		$this->benchmark->mark('oh_end');
		...
		// Fetching Opening hours
		$this->benchmark->mark('os_start');
		$order_settings = $this->get_order_settings($company_id,$general_settings[0]->calendar_country);
		$this->benchmark->mark('os_end');
		
		// Fetching Opening hours
		//$pre_assigned_holidays = $this->get_pre_assigned_holidays($company_id,$year,$month);
		
		// Fetching Opening hours
		$this->benchmark->mark('d_start');
		$days = $this->get_days();
		$this->benchmark->mark('d_end');
		
		// Fetching Countries
		$this->benchmark->mark('c_start');
		$countries = $this->get_countries();
		$this->benchmark->mark('c_end');
		
		// Fetching Available Payment Options for Cardgate
		$this->benchmark->mark('pay_start');
		$cardgate = $this->cardgate_payment_option($company_id);
		$this->benchmark->mark('pay_end');
		...
		$response_arr = array(
				...
				'cardgate'=>$cardgate,
				'allergence_words' => $allergence_words,
				'cat_start' => $this->benchmark->elapsed_time('cat_start', 'cat_end'),
				'subcat_start' => $this->benchmark->elapsed_time('subcat_start', 'subcat_end'),
				'ds_start' => $this->benchmark->elapsed_time('ds_start', 'ds_end'),
				'da_start' => $this->benchmark->elapsed_time('da_start', 'da_end'),
				'is_start' => $this->benchmark->elapsed_time('is_start', 'is_end'),
				'oh_start' => $this->benchmark->elapsed_time('oh_start', 'oh_end'),
				'os_start' => $this->benchmark->elapsed_time('os_start', 'os_end'),
				'd_start' => $this->benchmark->elapsed_time('d_start', 'd_end'), 
				'c_start' => $this->benchmark->elapsed_time('c_start', 'c_end'), 
				'cat_start' => $this->benchmark->elapsed_time('cat_start', 'cat_end'), 
				'time_gs' => $this->benchmark->elapsed_time('gs_start', 'gs_end'),
				'elapsed_time' => $this->benchmark->elapsed_time('code_start', 'code_end'),
				'pay_card' => $this->benchmark->elapsed_time('pay_start', 'pay_end'),
				'total_exec_time' => $this->benchmark->elapsed_time('all_start', 'all_end')
		);
...
// 			$international_areas = $this->db->get('company_countries')->result();
	...
	public function verify_user_post(){	
	   	...
	   	if( $client != null && isset($client->id) )
	   	{
	   		...
	   		if(!empty($clientInfo)){
	   			...
	   			$client->introcode_applied = $clientInfo['introcode_applied'];
		...
		else
		{
			...
			if( $client_id )
			{
				...
				if( $service_select == 'pickup' )
				{
					...
					$disc_client_amount_price = ( isset($post['disc_client_amount'])?$post['disc_client_amount']:0 );
					$disc_other_code_price = ( isset($post['disc_promo_code'])?$post['disc_promo_code']:0 );
					$disc_amount_price = ( isset($post['disc_amount'])?$post['disc_amount']:0 );
					$pickup_total = $total_cart_amount- ($disc_amount_price+$disc_other_code_price+$disc_client_amount_price);
					$pickup_total = ($post['c_tax_amount'] + ($pickup_total * ($post['c_tax_percentage']/100)) + $pickup_total);
					...
					$insert_arr = array(    
							...
							'disc_after_amount' => ( isset($post['disc_after_amount'])?$post['disc_after_amount']:0 ),
							'disc_promo_perc' => ( isset($post['disc_promo_perc'])?$post['disc_promo_perc']:0 ),
							'disc_promo_price' => ( isset($post['disc_promo_price'])?$post['disc_promo_price']:0 ),
							...
							'disc_other_code' => ( isset($post['disc_promo_code'])?$post['disc_promo_code']:0 ),
							'other_code_type' => ( isset($post['applied_code'])?$post['applied_code']:'' ),
				...
				elseif( $service_select == 'delivery' )
				{
					...
					$disc_other_code_price = ( isset($post['disc_promo_code'])?$post['disc_promo_code']:0 );
					$disc_amount_price = ( isset($post['disc_amount'])?$post['disc_amount']:0 );
					$delivery_cost_price = ( isset($post['rsDELPRICE'])?$post['rsDELPRICE']:0 );
					$delivery_total = ( $total_cart_amount + $delivery_cost_price )- ($disc_amount_price+$disc_other_code_price+$disc_client_amount_price);
					$delivery_total = ($post['c_tax_amount'] + ($delivery_total * ($post['c_tax_percentage']/100)) + $delivery_total);
				...
				if(!empty( $insert_arr ))
				{
					...
					if($insert_arr['payment_via_paypal']){
						...
						$associate = $this->associate_client_with_company( $client_id, $post['company_id'] );
						
						if(isset($post['applied_code']) && $post['applied_code'] == 'introcode') {
							$this->db->where(array('company_id'=>$post['company_id'], 'client_id' => $client_id));
							$this->db->update('client_numbers', array('introcode_applied' => 1));
						}
					}
					
					$this->response( array('error' => 0, 'message'=> '', 'data' => $order_id, 'pickup_total' => $pickup_total, 'delivery_total' => $delivery_total ), 200 );
				}
				else
				{
		...
		if( $order_details_id )
		{
			if( $send_mail )
			{
				...
				if(!empty($order_data)){
					...
					if(isset($order_data['delivery_city']) && $order_data['delivery_city'] != ''){
						$this->db->select('area_name');
						$city = $this->db->get_where('postcodes',array('id'=>$order_data['delivery_city']))->row();
						if($city != null){
							$order_data['delivery_city_name'] = $city->area_name;
						}
					}
	...
	function send_order_details_post( $order_id = 0, $client_id = 0, $order_data = array(), $order_details_data = array(), $ip_address = '' )
	{
		...
		if( !empty($client) && !empty($company) && !empty($company_gs) )
		{
			...
			/*if($client_id == 8)
				...
				$flag = send_email( $company_email, $this->config->item('no_reply_email'), stripslashes($orderreceived_subject), $mail_body, _('Order').' '.$company_name, NULL, NULL, 'no_reply', 'company', 'api_new_order_details');
	...
	function access_order_total_post(){
		...
		else
		{
			...
			$curr_time =$this->get('timestamp');
			$this->db->select('order_pickupdate,delivery_date,order_total');
			$order_check = $this->db->get_where('orders_tmp',array('id'=>$order_id))->result_array();
			
			$fp = fopen(APPPATH . "/controllers/new-api/logs_repayment.txt","a");
			fwrite($fp,json_encode(array('order_id'=>$order_id,'current_time'=>$curr_time)));
			fclose($fp);
			
			if ($order_check[0]['order_pickupdate'] == '0000-00-00')
			{
				$order_deliver_date = strtotime($order_check[0]['delivery_date']);
				if (($order_deliver_date == $curr_time) || ($order_deliver_date > $curr_time))
				{
					$this->db->select('company_id,delivery_hour,delivery_minute,order_total,delivery_cost,disc_client_amount,other_code_type,disc_other_code,disc_amount,disc_percent,disc_price');
					$order_delivertime = $this->db->get_where('orders_tmp',array('id'=>$order_id))->result_array();
					$order_delivertime = $order_delivertime[0]['delivery_hour'].':'.$order_delivertime[0]['delivery_minute'];
					$current_time = date('h:i', time());
	
					$datetime1 = new DateTime($order_check[0]['delivery_date'].' '.$order_delivertime);
					$datetime2 = new DateTime(date('Y-m-d h:i', time()));
					$interval = $datetime1->diff($datetime2);
					
					$disc_per_client_amount = 0; $disc_per_other_code = 0; $disc_per_amount = 0;
					$this->db->select('disc_per_amount, disc_after_amount');
					$company_gs = $this->db->where( array('company_id'=>$order_delivertime[0]['company_id']) )->get( 'general_settings' )->result();
						
					$order_total = $order_check[0]['order_total'];
					$order_total = $order_total + $order_delivertime[0]['delivery_cost'];
					
					if($order_delivertime[0]['disc_client_amount'] > 0)
					{
						$disc_per_client_amount = $order_delivertime[0]['disc_client_amount'];
					}
						
					if($order_delivertime[0]['other_code_type'] != '' && $order_delivertime[0]['disc_other_code'] != '0' )
					{
						$disc_per_other_code = $order_delivertime[0]['disc_other_code'];
					}
						
					if($company_gs[0]->disc_per_amount == 1 && $company_gs[0]->disc_after_amount > 0  && $order_delivertime[0]['disc_amount'] > 0 && ($order_delivertime[0]['disc_percent'] > 0 || $order_delivertime[0]['disc_price'] > 0))
					{
						$disc_per_amount = $order_delivertime[0]['disc_amount'];
					}
					$total_cost_after_discount = round($order_total,2)- (round($disc_per_amount,2)+round($disc_per_other_code,2)+round($disc_per_client_amount,2)) ;
					
					$this->check_order_total_valid($interval,$order_check,$total_cost_after_discount);
				}
				else
				{
						
					$order_data->order_total = $order_check[0]['order_total'];
					$order_data->order_valid = -1;
					$this->response( array('error' => 1, 'message'=> _('Order has been expired or does not exist.'), 'data' => $order_data), 404 );
				}
			}
			else {
				$order_pickup_date = strtotime($order_check[0]['order_pickupdate']);
				if (($order_pickup_date == $curr_time) || ($order_pickup_date > $curr_time))
				{
					$this->db->select('company_id,order_pickuptime,order_total,disc_client_amount,other_code_type,disc_other_code,disc_amount,disc_percent,disc_price');
					$order_pickuptime = $this->db->get_where('orders_tmp',array('id'=>$order_id))->result_array();
					$order_pkptime = $order_pickuptime[0]['order_pickuptime'];
					$current_time = date('h:i', time());
	
					$datetime1 = new DateTime($order_check[0]['order_pickupdate'].' '.$order_pkptime);
					$datetime2 = new DateTime(date('Y-m-d h:i', time()));
					$interval = $datetime1->diff($datetime2);
					
					$disc_per_client_amount = 0; $disc_per_other_code = 0; $disc_per_amount = 0;
					$this->db->select('disc_per_amount, disc_after_amount');
					$company_gs = $this->db->where( array('company_id'=>$order_pickuptime[0]['company_id']) )->get( 'general_settings' )->result();
					
					$order_total = $order_check[0]['order_total'];
						
					if($order_pickuptime[0]['disc_client_amount'] > 0)
					{
						$disc_per_client_amount = $order_pickuptime[0]['disc_client_amount'];
					}
					
					if($order_pickuptime[0]['other_code_type'] != '' && $order_pickuptime[0]['disc_other_code'] != '0' )
					{
						$disc_per_other_code = $order_pickuptime[0]['disc_other_code'];
					}
					
					if($company_gs[0]->disc_per_amount == 1 && $company_gs[0]->disc_after_amount > 0  && $order_pickuptime[0]['disc_amount'] > 0 && ($order_pickuptime[0]['disc_percent'] > 0 || $order_pickuptime[0]['disc_price'] > 0))
					{
						$disc_per_amount = $order_pickuptime[0]['disc_amount'];
					}
	...
	private function check_order_total_valid($interval = array(),$order_check = array(),$total_cost_after_discount = 0){
		if(($interval->format('%d') > '0') || ( $interval->format('%d') == '0' && $interval->format('%h') >= '3' && $interval->format('%i') >= '0'))
		{
			$order_data->order_total = $total_cost_after_discount;
			$order_data->order_valid = 1;
			$this->response( array('error' => 0, 'message'=>_('Success'), 'data' =>  $order_data), 200 );
	...
	function edit_user_profile_post()
	{
		...
		else
		{
			...
			$update_array = array(
					...
					'address_ci' => $this->post('address_ci'),
					'housenumber_ci' => $this->post('housenumber_ci'),
					'busnummeri' => $this->post('busnummeri'),
					'postcode_ci' => $this->post('postcode_ci'),
					'city_ci' => $this->post('city_ci'),
	...
	function get_order_details_post()
	{
		...
		if( $order != null )
		{
			...
			if( $area_name != null && isset( $area_name->area_name ) )
			...
			if( $city_name != null && isset( $city_name->city_name ) )
	 ...
	 * This function is used to get order detail of given order ID
	 */
	function get_order_details_cart_post()
	{
		$order_id = $this->post('order_id');
		...
		if( $order != null )
		{
			...
			if(!empty($order_details)){
				foreach ($order_details as $pkey=>$p_val){
					$wUnit = $p_val->weight_unit;
					$group_additional = 0;
					$groups_selected = array();
					$qty = 0;
					$sell_product_option = $p_val->content_type;
					
					if($p_val->add_costs != ''){
						
						$add_costs = $p_val->add_costs;
						$grp_name = array();
						$grp_val = array();
						if( $add_costs != '' ){
							$add_costs_arr = explode('#',$add_costs);
							if(!empty($add_costs_arr))
							foreach( $add_costs_arr as $ac ){
								$ac = explode('_',$ac);
						...
						$grp_arr = $this->get_product_groups($p_val->product_id);
						
						foreach ($grp_arr as $gval){
							if(in_array($gval['grp_name'], $grp_name)){
								foreach ($gval['attributes_arr'] as $aval){
									if(in_array($aval[0], $grp_val)){
										$groups_selected[] = $gval['grp_name']."_".$aval[0]."_".$aval[1];
										$group_additional += floatval($aval[1]);
									}
								}
							}
						}
					...
					$qty = intval($p_val->quantity);
					
					if($sell_product_option == 1){
						if($wUnit == 'kg'){
							if((floatval($p_val->min_weight) != 0) && (floatval($p_val->quantity) < floatval($p_val->min_weight)))
								$qty = floatval($p_val->min_weight);
							elseif((floatval($p_val->max_weight) != 0) && (floatval($p_val->quantity) > floatval($p_val->max_weight)))
								$qty = floatval($p_val->max_weight);
							else
								$qty = floatval($qty);
						}
						elseif($wUnit == 'gm'){
							if((floatval($p_val->min_weight) != 0) && (intval($p_val->quantity) < intval($p_val->min_weight * 1000)))
								$qty = intval($p_val->min_weight * 1000);
							elseif((floatval($p_val->max_weight) != 0) && (intval($p_val->quantity) > intval($p_val->max_weight * 1000)))
								$qty = intval($p_val->max_weight * 1000);
						}
					}
					elseif($sell_product_option == 2){
						if((intval($p_val->min_amount) != 0) && (intval($p_val->quantity) < intval($p_val->min_amount))){
							$qty = intval($p_val->min_amount);
						}
						elseif((intval($p_val->max_amount) != 0) && (intval($p_val->quantity) > intval($p_val->max_amount))){
							$qty = intval($p_val->max_amount);
						}
					}
					...
					$total_amount = 0;
					$net_price = 0.00;
					$add_costs = implode('#',$groups_selected);
					$total_amount = 0;
					$apply_discount = 0;
					$default_price = 0;
					$net_price_in_kg = 0;
					...
					if( $sell_product_option == 1 ){
						if($wUnit == "kg")
							$net_quantity = ( $qty * 1000 );
						
						$net_price = floatval($p_val->price_weight); // Net Price In gm (0.02 euro / gm = 20 euro / KG )
						$default_price = floatval($p_val->price_weight); // Net Price In gm (0.02 euro / gm = 20 euro / KG )
						$net_price_in_kg = $net_price*1000; // Net Price In KG (20 euro / KG )
						$content_type = 1;
						$product_discount = $p_val->discount_wt;
						
						if($p_val->discount_wt == "multi"){
							$product_unit_type = 1;
							$apply_discount_unit = 0;
							$apply_discount = 0;
							$discount = $this->get_product_multi_discounts($p_val->product_id,'1');
								
							/*======processing each discount to check if quantity matches to the discount quantity======*/
							for($j=0; $j < count($discount); $j++){
								if( $discount[$j]->type == $product_unit_type ){
									$quantity_in_comp_discount_unit = intval($net_quantity/$discount[$j]->quantity);
										
									if($quantity_in_comp_discount_unit > 0 && intval($discount[$j]->quantity) > $apply_discount_unit){
										$apply_discount = floatval($discount[$j]->discount_per_qty);
										$apply_discount_unit = intval($discount[$j]->quantity);
										$net_price = floatval($discount[$j]->price_per_qty); //per gram
									}
								}
							}
								
							if($apply_discount && $apply_discount_unit){
								$comp_dis_unit = intval($net_quantity/$apply_discount_unit);
								$rem_dis_unit = intval($net_quantity % $apply_discount_unit);
						...
						}elseif($p_val->discount_wt != 0 && floatval($p_val->discount_wt) == $p_val->discount_wt ){
							$quantity_in_comp_kg = intval($net_quantity/1000);
							$quantity_left_in_gm = ($net_quantity%1000);
								
							if($quantity_in_comp_kg > 0){
								$apply_discount = floatval($p_val->discount_wt);
								$net_price_in_kg = $net_price_in_kg - $apply_discount;
								$net_price = $net_price_in_kg/1000;
							}
								
							$comp_tot_amount = floatval($quantity_in_comp_kg * 1000 * $net_price);
							$incomp_tot_amount = floatval($quantity_left_in_gm * $default_price);
							$total_amount = $comp_tot_amount + $incomp_tot_amount;
						}
						else{
							$total_amount = $net_quantity * $net_price;
						...
						$total_amount = floatval($total_amount) + floatval($group_additional);
	
					}else if( $sell_product_option == 2 ){
						$net_price = floatval($p_val->price_per_person);
						$default_price = floatval($p_val->price_per_person);
						$content_type = 2;
						$product_discount = $p_val->discount_person;
						
						if($p_val->discount_person == "multi"){
							$product_unit_type = 2;
							$apply_discount_unit = 0;
							$apply_discount = 0;
							$discount = $this->get_product_multi_discounts($p_val->product_id,'2');
							
							/*======processing each discount to check if quantity matches to the discount quantity======*/
							if(count($discount)){
								for($j = 0; $j < count($discount); $j++){
									if( intval($discount[$j]->type) == $product_unit_type ){
										if( intval($discount[$j]->quantity) <= intval($net_quantity) ){
											$apply_discount = floatval($discount[$j]->discount_per_qty);
											$net_price = floatval($discount[$j]->price_per_qty);
										}
									}
								}
							}
						}elseif($p_val->discount_person != 0 && floatval($p_val->discount_person) == $p_val->discount_person){
							$net_price = $net_price - floatval($p_val->discount_person);
						}
	
						$total_amount = intval($net_quantity)*( floatval($net_price) + floatval($group_additional) );
					}
					else{
						$net_price = floatval($p_val->price_per_unit);
						$default_price = floatval($p_val->price_per_unit);
						$content_type = 0;
						$product_discount = $p_val->discount_unit;
						
						if($p_val->discount_unit == "multi"){
							$product_unit_type = 0;
							
							$discount = $this->get_product_multi_discounts($p_val->product_id,'0');
							
							/*======processing each discount to check if quantity matches to the discount quantity======*/
							if(count($discount)){
								for($j = 0; $j < count($discount); $j++){
									if( $discount[$j]->type == $product_unit_type ){
										if($discount[$j]->quantity <= intval($net_quantity) ){
											$apply_discount = floatval($discount[$j]->discount_per_qty);
											$net_price = floatval($discount[$j]->price_per_qty);
										}
									}
						...
						}elseif($p_val->discount_unit != 0 && floatval($p_val->discount_unit) == $p_val->discount_unit){
							$net_price = $net_price - floatval($p_val->discount_unit);
						}
						$total_amount = intval($net_quantity)*( floatval($net_price) + floatval($group_additional) );
					}
					
					$order_details[$pkey]->price_weight = ((float)$order_details[$pkey]->price_weight)*1000;
					$order_details[$pkey]->quantity = $qty;
					$order_details[$pkey]->discount = $p_val->discount_unit;
					$order_details[$pkey]->default_price = $default_price;
					$order_details[$pkey]->sub_total = $net_price;
					$order_details[$pkey]->add_costs = $add_costs;
					$order_details[$pkey]->total = $total_amount;
				}
			}
				
			$total_cart_amount = 0;
			if( count($order_details) ){
				for( $counter = 0; $counter < count($order_details); $counter++){
					$total_cart_amount = $total_cart_amount + floatval($order_details[$counter]->total);
				}
			}
				
			$order_detail_arr['order']->order_total = $total_cart_amount;
	...
	function get_order_repay_post(){
		...
		if( $order != null ){
			...
			$this->response( array('error' => 0, 'message'=> '', 'data' => array('order' => $order, 'order_detail' => $order_detail_arr) ), 200 );
		}
		else{
			$this->response( array('error' => 1, 'message'=>_('Can\'t fetch this order detail.'), 'data' => '' ), 404 );
		}
	...
	// 			$verfication_code = random_string('numeric', 5);
				...
				$registration_array=array(
						...
						'newsletters' => ( $newsletters == 'subscribe' )?'subscribe':'unsubscribe',
						'notifications' => ( $notifications == 'unsubscribe' )?'unsubscribe':'subscribe',
						'company_c' => $this->post('company_c'),
						'vat_c' => $this->post('vat_c'),
						'company_id' => $this->post('company_id'),
						'address_ci' => $this->post('address_ci'),
				...
				if($this->db->affected_rows())
				{
					...
					$discount_card_array=array('discount_card_number' => ($this->post('discount_card_number'))?$this->input->post('discount_card_number'):'',
							'client_id'=>$client_id,
	...
	function send_user_query_post(){
		if($this->post('action') == 'send_user_query' ){
			$sender		= $this->post('sender');
			$subject	= trim($this->post('subject'));
			$msg		= $this->post('message');
			$comp_id	= $this->post('company_id');
				
			if($sender && $subject && $msg && $comp_id){
				$this->db->select('emailid');
				$eresult = $this->db->where( array('company_id'=>$comp_id) )->get( 'general_settings' )->row();
				
				if($eresult != null && $eresult->emailid != ''){
					$msg = nl2br($msg);
					send_email( $eresult->emailid , $sender , $subject , $msg , 'Via webshop' , NULL , NULL , 'client' , 'company' , 'query_by_client' );
					
					$this->response( array('error' => 0, 'message'=> '', 'data' => $comp_id ), 200 );
				}
				else{
					$this->response( array('error' => 1, 'message'=>_('Sorry ! Your query can not be submitted.'), 'data' => '' ), 404 );
				}
			}
			else
				$this->response( array('error' => 1, 'message'=>_('Sorry ! Your query can not be submitted.'), 'data' => '' ), 404 );
	...
	function delete_order_post(){
		...
		if($this->db->update('orders', array('hidden' => true))){
			...
			$this->db->select('orders.*,delivery_country.country_name as delivery_country_name');
			$this->db->join('country as delivery_country','orders.delivery_country = delivery_country.id','left');//used in delivery address
			$order_data = $this->db->where( array('orders.id'=>$order_id,'clients_id'=>$user_id) )->get( 'orders' )->row_array();

			if( ( $order_data['option'] == '1' && $order_data['order_pickupdate'] >= date('Y-m-d') ) || ( $order_data['option'] == '2' && $order_data['delivery_date'] >= date('Y-m-d') ) ){
				...
				$this->db->select('clients.id, clients.company_c, clients.firstname_c, clients.lastname_c, clients.address_c, clients.housenumber_c, clients.postcode_c, clients.city_c, clients.vat_c, clients.phone_c, clients.email_c, country.country_name as country_c');
				$this->db->join('country','country.id = clients.country_id','left');
				$client = $this->db->where( array('clients.id'=>$user_id) )->get( 'clients' )->row();

				$company = $this->db->where( array('id'=>$order_data['company_id']) )->get( 'company' )->row();

				$client_discount_number = $this->db->where( array('id'=>$user_id , 'company_id' => $order_data['company_id']) )->get( 'client_numbers' )->result();
				$c_discount_number = '--';
				if(!empty($client_discount_number)){
					$c_discount_number = $client_discount_number['0']->discount_card_number;
				}
				...
				if( !empty($client) && !empty($company) && !empty($company_gs) ){
					...
					$mail_body = '';
					if(!empty($delivery_settings) && $delivery_settings[0]->type == 'international'){
						$mail_body = $this->load->view( 'mail_templates/order_cancel_by_user_mail', $mail_data, true );
					}
					else{
						$mail_body = $this->load->view( 'mail_templates/order_cancel_by_user_mail', $mail_data, true );
					}

					send_email( /*$company_email*/'abhayhayaran@cedcoos.com', $this->config->item('no_reply_email'), stripslashes($orderreceived_subject), $mail_body, _('Order').' '.$company_name, NULL, NULL, 'no_reply', 'company', 'api_new_order_details');
				}
			}
	...
	/*
	* Function for update shop json files
	*/
