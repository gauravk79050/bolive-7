...
class Image_upload extends CI_controller{
	 ...
	 * @name $baz
	 */
	function ajax_img_upload($view_page = 'cp',$num = 0,$width=0){
		//echo "abc";die;
		$data['attr_det'] = '';
	 ...
	 * @return string An error message or an HTML of containing image and some information
	 */
	function ajax_gallery_image_upload($num = 1,$set_width=0){
		$path = dirname(__FILE__).'/../../../assets/temp_uploads/';
		$valid_formats = array("jpg", "png", "gif", "bmp", "jpeg", "JPG", "PNG", "GIF");
		if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST"){
			$name 		= $_FILES['photoimg']['name'];
			$name_arr 	= explode( '.', $name );
			$name		= clean_pdf( $name_arr[ 0 ] );
			$name		= $name.'.'.$name_arr[ sizeof( $name_arr ) - 1 ];
			//echo $name;die;
			$size 		= $_FILES['photoimg']['size'];
			// echo $name;die;
			if(strlen($name)){
	 ...
	 */
	function crop_image($num = 0,$set_width=0){
		...
		if (isset($shop_result[0]) && $shop_result[0]['biggest_image'])
		{
			$maxWidth = 600;
			$maxHeight = 600;
		}else
		...
		$aller_size = getimagesize(dirname(__FILE__).'/../../../assets/temp_uploads/cropped_'.$image);
		$new_html = "";

		if($num){
			if( $num == 5 ) {
				$new_html .= "<img src='".base_url()."assets/temp_uploads/cropped_".$image."' alt='"._("Sorry !!! image has not been cropped. Please try again")."' style='float: left; margin-right:20px; max-width:800px; max-height:300px;' /> <input type='hidden' name='image_name".$num."' id='image_name".$num."' value='cropped_".$image."' />";
				$new_html .= '<span>Transparency:</span>&nbsp <div class="brightness_slider"></div><input type="hidden" value="0" name="transparency" id="transparency"><span class="brightness_percent"></span><input type="hidden" value="'.$aller_size[0].'" id="img_width"><input type="hidden" value="'.$aller_size[1].'" id="img_height">';
			} else {
