<?php
	class Autocontrole extends CI_Controller
	{
	   ...
	   function __construct()
	   {
		  ...
		  $this->load->model( 'mcp/Mautocontrole' );	
		  $rp_user_id = $this->session->userdata( 'rp_user_id' );
		
		  if( !$rp_user_id || $rp_user_id != '10' )
	    ...
	    */
	    ...
	    function categories( )
	    {
		   ...
		   //$this->load->vars( $data );
		   $this->load->view( $this->template.'rp_view', $data );	
	   }
	   ...
	   function autocontrole_categories_addedit( ){
	   		...
	   		//add type
		        ...
		        redirect( base_url().'rp/autocontrole/categories' );
	    ...
	    */
	    ...
	    /**
	   	* to insert manually predefined to existing predef.
	   	**/
	   	function merge_predefined(){
	   		$type_id 	= $this->input->post( 'type_id' );
	   		$pred_id 	= $this->input->post( 'pred_id' );
	   		...
	   		$result = $this->Mautocontrole->update_task( $id, $update_arr );
	   		if( $result ){
	   			echo "success";
	   			exit();
	   		} else {
	   	...
	   	function addeditPredifined( ){
	   		if( $this->input->post( 'action' ) == 'ADD Predifined' ){ 										//ADD Predifined
		        ...
		        if( $data[ 'type_id' ] == 'Select Type'){
					...
					redirect( base_url().'rp/autocontrole/predifined' );
		        }
	    ...
	    */
	   	function updatePredifined( ){
	   		...
	   		$redirect_url = base_url().'rp/autocontrole/predifined';
			if( $data[ 'action' ] == 'Save and next' ){
				...
				if( isset(  $prev_n_nxt_id['next_id'] ) ){
					$redirect_url = base_url().'rp/autocontrole/addeditPredifined/'. $prev_n_nxt_id['next_id'];
				}
			} 
	   		unset( $data[ 'action' ] );
	   		unset( $data[ 'id' ] );
	   ...
	   function searchPredifined( ){
	   		...
	   		if( !empty( $result[ 'types' ] ) ){
	   			$type_ids = array( );
		   		if( !empty( $result[ 'types' ] ) ){
		   			$type_ids = array_values( array_unique( array_column( $result[ 'types' ] , 'type_id' ) ) );
		   		}
		   		for( $i=0; $i < sizeof( $type_ids ); $i++ ){		// making an array that has all simillar type predifined together
		   			$final_arr[ $i ] = array( );
		   			foreach ( $result[ 'autocontrole_predifined' ] as $key => $predifined ) {
	   ...
	   function addeditLocation( ){
	   		if( $this->input->post( 'action' ) == 'ADD LOCATION' ){ //ADD Predifined
		        ...
		        if( $result ){
		   		...
		   		redirect( base_url().'rp/autocontrole/location' );
		    }else{
		   			...
		   			foreach ($upload_arr as $key => $value) {
			            ...
			            $this->upload->initialize( $config );
					  	if ( ! $this->upload->do_upload( 'mediafile' ) ){
					  		$this->session->set_flashdata( 'msg',$this->upload->display_errors( ) );
							redirect( base_url().'rp/autocontrole/procedure' );
					  	}
		   			}
			   	}else{
	    ...
	    */
	   	...
	   	function updateObjects() {
	   		$redirect_url 		= base_url().'rp/autocontrole/objects';
	   		...
	   		if( isset(  $post_data[ 'action' ] ) && $post_data[ 'action' ] == 'Save and next' ){
	   			if( isset( $post_data['id']) && $post_data['id'] != '' ){
					...
					if( isset(  $prev_n_nxt_id['next_id'] ) ){
						$redirect_url = base_url().'rp/autocontrole/editObjects/'. $prev_n_nxt_id['next_id'];
					}
	   	...
	   	function searchObjects() {
	   		...
	   		if( $search_type == '' ){
	   			$this->session->set_flashdata( 'msg', _( 'Please provide Search By' ) );
	   			redirect( base_url().'rp/autocontrole/objects' );
	   		}
	   ...
	   function addeditcategory(){
	   	if( $this->input->post( 'action' ) == 'add category' ){
			    ...
			    if( $result ){
			   	...
			   	redirect( base_url().'rp/autocontrole/category' );
		    }else{
	   ...
	   function domain_addedit( ){
	   		if( $this->input->post( 'action' ) == 'ADD DOMAIN' ){ 										//add type
		        ...
		        redirect( base_url().'rp/autocontrole/domains' );
	   ...
	   function checklist_add_edit( ){
	   		if( $this->input->post( 'action' ) == 'ADD CHECKLIST' ){ 										//ADD Predifined
		        ...
		        if( $result ){
		   		...
		   		redirect( base_url().'rp/autocontrole/checklist' );
		    }else{
	   ...
	   function updateChecklist( ){
		...
		function addedit_objects_category( ){
			if( $this->input->post( 'action' ) == 'add category' ){ 										//add type
		        ...
		        redirect( base_url().'rp/autocontrole/object_categories' );
	    ...
	    */
		...
		function update_temp_group() {
			...
			if( $result ){
	   		...
	   		redirect( base_url().'rp/autocontrole/temperature_group/'.$country_code );
		}	   

		 /**
	    *  Function to delete temperature group
	    */
	   	...
	   	function ccp_pva_ghp_add_edit( $id = '', $country_code = '' ){
			...
			if( $data[ 'kind_of_danger' ] != '' ){
	   		...
	   		if( $data[ 'action_btn' ] == "ADD CCP PVA GHP" ){
	   			...
	   			redirect( base_url().'rp/autocontrole/ccp_pva_ghp/'.strtolower( $data[ 'country_code' ] ) );
			...
			$data[ 'allTask' ] 			    = $result[ 'allTask' ];
	   		$data['header']					= $this->template.'header';
			$data['main']					= $this->mcp_template.'autocontrole_ccp_pva_ghp_addEdit';
			$data['footer']					= $this->template.'footer';
			$this->load->vars( $data );
			$this->load->view($this->template.'rp_view');
	   	}
   		...
   		function ccp_pva_ghp( $country_code = '' ){
   			if( $country_code == 'be' || $country_code == 'nl' ) {
				...
				if( $this->input->post( 'btn_search' ) ){
	   		   ...
	   		   $data[ 'company_type' ] 			= $this->Mautocontrole->get_all_company_type();
	   		   $data['header'] 					= $this->template.'header';
			   $data['main']					= $this->mcp_template.'autocontrole_ccp_pva_ghp_list';
			   $data['footer']					= $this->template.'footer';
			   $this->load->view( $this->template.'rp_view', $data );
		   } else {
		   		if( isset( $_SERVER['HTTP_REFERER'] ) ) {
		 ...
		 * @author Abhishek Singh 
   		...
   		function delete_ccp_pva_ghp( $id ){
   			...
   			redirect( base_url().'rp/autocontrole/ccp_pva_ghp' );
   		}
		 ...
		 * @param  $id int 
   		...
   		function module( ){
   			...
   			$data['header']		= $this->template.'header';
		    $data['main']		= $this->mcp_template.'autocontrole_module_list';
		    $data['footer']		= $this->template.'footer';
		    //$this->load->vars( $data );
		    $this->load->view( $this->template.'rp_view', $data );
   		}
	   ...
	   function addeditmodule( ){
	   		...
	   		if( $this->input->post( 'action' ) == 'ADD MODULE' ){
	   			foreach ($upload_arr as $key => $value) {
			   		if( $_FILES[ $value ][ 'name' ] != '' ){
			            ...
			            $this->upload->initialize( $config );
					  	if ( ! $this->upload->do_upload( 'mediafile' ) ){
					  		$this->session->set_flashdata( 'msg',$this->upload->display_errors( ) );
							redirect( base_url().'rp/autocontrole/module' );
					  	}
				   	}else{
			   			$this->session->set_flashdata( 'msg', _( 'Please provide a file to upload.' ) );
				   		redirect( base_url().'rp/autocontrole/module' );
		    	...
		    	if( !empty( $result ) ){
		   		...
		   		$data['header']		= $this->template.'header';
				$data['main']		= $this->mcp_template.'autocontrole_modulesAddEdit';
				$data['footer']		= $this->template.'footer';
				//$this->load->vars( $data );
				$this->load->view($this->template.'rp_view', $data);
		    }	
	    ...
	    */
	   	function addedit_pasteur_group( $id = '', $country_code = '' ){
	   		if( is_numeric( $id ) ){ 													
				...
				redirect( base_url( 'rp/autocontrole/pasteurization_group' ) );
			}
		...
		function update_pasteur_group() {
			...
			if( $result ){
	   		...
	   		redirect( base_url().'rp/autocontrole/pasteurization_group/'.strtolower( $data[ 'country_code' ] ) );
		}	   
		   ...
		   $data['header']	= $this->template.'header';
		   $data['main']	= $this->mcp_template.'autocontrole_workroom_list';
		   $data['footer']	= $this->template.'footer';
		   //$this->load->vars( $data );
		   $this->load->view( $this->template.'rp_view', $data );
	   	}
	   	...
	   	function addedit_workroom( $id = '' ){
	   		if( is_numeric( $id ) ){ 													
				...
				redirect( base_url( 'rp/autocontrole/temperature_workroom' ) );
			}
			
			$data['header']	= $this->template.'header';
			$data['main']	= $this->mcp_template.'addedit_workroom';
			$data['footer']	= $this->template.'footer';							
	        //$this->load->vars( $data );
			$this->load->view($this->template.'rp_view', $data);
	   	}
		...
		function delete_workroom() {
		   ...
		   $data['header']	= $this->template.'header';
		   $data['main']	= $this->mcp_template.'autocontrole_calibration';
		   $data['footer']	= $this->template.'footer';
		   //$this->load->vars( $data );
		   $this->load->view( $this->template.'rp_view', $data );
	   	}
	   	...
	   	function addedit_calibration( $id = '' ){
			...
			$data['header']	= $this->template.'header';
			$data['main']	= $this->mcp_template.'addedit_calibration';
			$data['footer']	= $this->template.'footer';							
	        //$this->load->vars( $data );
			$this->load->view($this->template.'rp_view', $data);
	   	}
	   	...
	   	function update_calibration() {
			...
			if( $result ){
	   		...
	   		redirect( base_url().'rp/autocontrole/autocontrole_calibration' );
		}
		 ...
		 */
		   ...
		   $data['header']	= $this->template.'header';
		   $data['main']	= 'mcp/autocontrole_ph_groups';
		   $data['footer']	= $this->template.'footer';
		   //$this->load->vars( $data );
		   $this->load->view( 'mcp/mcp_view', $data );
	   	}
	   	...
	   	function addedit_ph_groups( $id = '' ){
			...
			$data['header']	= $this->template.'header';
			$data['main']	= 'mcp/addedit_ph_groups';
			$data['footer']	= $this->template.'footer';							
	        //$this->load->vars( $data );
			$this->load->view('mcp/mcp_view', $data);
	   	}
	   	...
	   	function update_ph_groups() {
			...
			if( $result ){
	   		...
	   		redirect( base_url().'rp/autocontrole/autocontrole_ph_groups' );
		}
		...
		function delete_ph_groups() {
	   	...
	   	/*=====  End of Autocontrole calibration  ======*/
