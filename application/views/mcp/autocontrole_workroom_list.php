...
if( strpos( current_url(), 'rp' ) > -1 ) {
  $edit_url = base_url().'rp/autocontrole/addedit_workroom/';
  $add_url = base_url().'rp/autocontrole/addedit_workroom';
} else {
  $edit_url = base_url().'mcp/autocontrole/addedit_workroom/';
  $add_url = base_url().'mcp/autocontrole/addedit_workroom';
}?>
...
<script type="text/javascript">
  ...
  $( document ).on( 'click','.delete_workroom' , function( ){ 
    ...
    if( confirm( delete_auto_temp_group ) ){
      $.ajax({
        url : base_url + section+'/autocontrole/delete_workroom',
        data: {
          'id' : id,
        },
        async : !1,
