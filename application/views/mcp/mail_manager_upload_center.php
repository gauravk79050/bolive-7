...
function do_upload(obj){
	...
	jQuery("#imageform").ajaxSubmit({
		...
		url: '<?php echo base_url()?>mcp/mail_manager/image_manager',
		type:'post',
...
$(document).ready(function(){
	$('.remove_me').on('click',function(){
		if(confirm("Are you sure you want to delete this image?")){
			...
			$.post('<?php echo base_url(); ?>mcp/mail_manager/delete_image',{'action':'delete','image_name':image_name},function(data){
				if(data.success){
					$(parent_ref).remove();
					alert(data.success);
				}
...
.remove_me{
	...
	right: 0;
      ...
      <div class="box" style="width: 98% !important;">	
        ...
        <div class="post">
			...
			<div style="text-align: center; background-color: #2E3134; padding: 20px;">
			  <form id="imageform" method="post" enctype="multipart/form-data" action='<?php echo base_url()?>mcp/mail_manager/ajax_image_upload'>
			...
			<div id="uploaded_images" style="overflow: hidden;">
				<?php //print_r($images); ?>
				<?php foreach($images as $k=>$image){
					$image_div = '';
					$image_div .= '<div class="image_holder" image_name='.$image['image_name'].'>';
					$image_div .= '<img src="'.base_url().'assets/upload_center/images/'.$image['image_name'] .'" alt="image_'.$k.'" width="150px" height="150px" style="padding:5px;" />';
...
boundy,xsize,ysize,$preview,$pcnt,$pimg;
  ...
  function updatePreview(c)
  {
	...
	function crop(){
		...
		$.ajax({
			url : base_url+'mcp/mail_manager/crop_upload_center_image',
