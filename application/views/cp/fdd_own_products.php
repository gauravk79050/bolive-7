...
<html>
	<head>
		<script type="text/javascript">
			...
			//var tags3 = <?php echo json_encode($producers); ?>;
			//var tags5 = <?php echo json_encode($suppliers); ?>;
			...
			$(document).ready(function(){
					...
					jQuery.ajax({
						...
						success: function(response){
							...
							for(var y=0;y<response.length;y++){
								...
								$('#tr_'+response[y].id).children('td').children('span').children('.semi_prdc_name').append('<span><input type="text" class="pro_quant2s text" style="width:6%" value="'+total_semi_quant.toFixed(2)+'" onkeyup="calculate_extra_semi_total(this,\''+unit_semi+'\')" ><strong> '+unit_semi+' </strong></span>');
							}
						}
					});
				}
				...
				$(".extra_semi_products_filter").keyup(function(e){
					if(e.keyCode != 37 && e.keyCode != 38 && e.keyCode != 39 && e.keyCode != 40){
						show_extra_semi_products_filter_suggestion();
					}
				});
					...
					<tr>
						<td width="10%" style="vertical-align: top;">
							<select onchange="change_product_db(this.value)" style="padding: 5px;width:115px" id="recipe_option">
								...
								<?php if ($page_id != 2){?>
								<option value="3"><?php echo _('Extra semiproduct');?></option>
								<?php }?>
...
<!-- 					<td id="add_semi_products" width="90%" style="display: none">
							<input id="extra_semi_search_box" style="width: 72%" class="text extra_semi_products_filter" type="text" placeholder="<?php echo _("Search by semi product's name");?>" name="product_name" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<input id="quant3" style="width: 9%" class="text quant3" type="number" placeholder="<?php echo _("Quantity");?>" name="quantity" min="0" max="100" >&nbsp;&nbsp;
									<!-- <div style="display: inline-block; width:25px;height:25px">
									<img id="loding_gif" alt="loading" src="<?php echo  base_url()."assets/images/loading2.gif"?>" style="display:none;width: 22px; margin-top: 2px;">
								</div>  -->
<!-- 						<img class="my_fdd_img" onclick="add_semi_extra_row()" alt="<?php echo _("add");?>" src="<?php echo base_url().'assets/images/plus_btn.png';?>" >&nbsp;&nbsp;
						</td>
-->
						<td id="add_semi_products" width="90%" style="display: none">
							...
							<img class="my_fdd_img" onclick="add_row3()" alt="<?php echo _("add");?>" src="<?php echo base_url().'assets/images/plus_btn.png';?>" >&nbsp;&nbsp;
							<div class="clear"></div>
						</td>
					</tr>
					<tr id="alert-msg" style="display:none">
				...
				<?php if(isset($total_used_pro_own) && !empty($total_used_pro_own)){?>
					...
					<tr id="tr_<?php echo $used_pro['fdd_pro_id'];?>" class="product_row2" rel="<?php echo $used_pro['quantity'];?>">
						...
						<?php }
							...
							<span><img onclick="remove_row(<?php echo $used_pro['fdd_pro_id'];?>)" class="my_fdd_img" src="<?php echo base_url().'assets/images/delete_pro.png';?>"></span>&nbsp;&nbsp;<span><input type="text" class="pro_quant2 text" style="width:6%" value="<?php echo str_replace($search, $replace,round($used_pro['quantity'],1));?>" onkeyup="calculate_total()" > <strong> <?php echo $used_pro['unit']; ?> </strong></span><span class="prdc_name"><?php echo stripslashes($used_pro['proname'].' '.'--'.' '.$used_pro['s_name']);?></span>
						<?php }?>
							<input type="hidden" class="hidden_pro_prefix2" value="<?php echo $used_pro['product_prefix'];?>">
							<input type="hidden" class="hidden_semi_pro_id" value="<?php echo $used_pro['semi_product_id'];?>">
						</td>
					</tr>
					<?php }?>
