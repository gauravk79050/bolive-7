	...
	<body>
		...
		<div id="container" style="margin:5px;padding:5px;">
			...
			<div id="right_div" style="width:60%;margin:5px;padding:5px;float:left" >
				...
				<br/>
				<p><b><?php echo _('OVERVIEW').' ('._('Total weight').' : '.$product_information[0]->recipe_weight. _(' kg').')'; ?></b></p>

				<table>
				<?php if(isset($semi_pro_array) && !empty($semi_pro_array)){?>
					<?php foreach ($semi_pro_array as $semi_pro_arr){?>
							<tr>
								<td ><b><?php echo round($semi_pro_arr['quant'],0)._('gr');?></b></td>
								<td ><b><?php echo stripslashes($semi_pro_arr['semi_name']);?></b></td>
								<td ></td>
							</tr>

							<?php if(isset($semi_contains) && !empty($semi_contains)){?>
								<?php foreach ($semi_contains as $semi_contain){?>

									<?php if($semi_pro_arr['semi_id'] == $semi_contain['semi_product_id']){?>
										<tr>
											<td style="background: pink"><?php echo round($semi_contain['quantity'],1)._('gr');?></td>
											<td style="background: pink"><?php echo stripslashes($semi_contain['p_name']);?></td>
											<td style="background: pink"><?php echo $semi_contain['fabrikant'];?></td>
										</tr>
									<?php }?>
								<?php }?>
							<?php }?>
				...
				<?php }?>
					...
					<?php if(isset($fdd_contains) && !empty($fdd_contains)){?>
						<?php foreach ($fdd_contains as $own_contain){?>
							<tr>
								<td><?php echo round($own_contain['quantity'],1)._('gr');?></td>
								<td><?php echo stripslashes($own_contain['p_name']);?></td>
								<td><?php echo $own_contain['fabrikant'];?></td>
							</tr>
						<?php }?>
					<?php }?>
					<?php if(isset($own_contains) && !empty($own_contains)){?>
						<?php foreach ($own_contains as $semi_contain){?>
							<tr>
								<td><?php echo round($semi_contain['quantity'],1)._('gr');?></td>
								<td><?php echo stripslashes($semi_contain['p_name']);?></td>
								<td><?php echo $semi_contain['fabrikant'];?></td>
							</tr>
						<?php }?>
					<?php }?>
				</table>
				...
				<br/>
				<p><b><?php echo _('METHOD');?></b></p>
				<p><?php echo nl2br($product_information[0]->recipe_method); ?></p>
