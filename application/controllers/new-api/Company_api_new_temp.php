 ...
 */
class Company_api_new_temp extends REST_Controller
{
	/**
	 * Defining variable
	 */
...
// 			$international_areas = $this->db->get('company_countries')->result();
	...
	function access_order_total_post(){
			...
			$total_cost_after_discount = defined_money_format((round($order_total,2)- (round($disc_per_amount,2)+round($disc_per_other_code,2)+round($disc_per_client_amount,2)) ));
			$this->response( array('error' => 0, 'message'=>_('Success'), 'data' =>  $total_cost_after_discount), 200 );
		...
		if( !empty($order) )
		{
			...
			if(!empty($order_details)){
				foreach ($order_details as $pkey=>$p_val){
					...
					if($sell_product_option == 2){
	
						if(intval($p_val->quantity) < intval($p_val->min_amount) )
							$qty = intval($p_val->min_amount);
					}
						
					if($sell_product_option == 1 && $wUnit == 'kg'){
						$qty = floatval($qty);
					}
					else{
						$qty = intval($qty);
					...
					if( $sell_product_option == 1 ){
						...
						if($p_val->discount_wt == "multi"){
							...
							for($j=0; $j < count($discount); $j++){
								if( $discount[$j]->type == $product_unit_type ){
									...
									if($quantity_in_comp_discount_unit > 0 && $discount[$j]->quantity > $apply_discount_unit){
										$apply_discount = floatval($discount[$j]->discount_per_qty);
										$apply_discount_unit = $discount[$j]->quantity;
						...
						$content_type = 0;
						$product_discount = $p_val->discount;
							
						if($p_val->discount == "multi"){
							$product_unit_type = 0;
						...
						}elseif($p_val->discount != 0 && floatval($p_val->discount) == $p_val->discount){
							$net_price = $net_price - floatval($p_val->discount);
						}
						$total_amount = intval($net_quantity)*( floatval($net_price) + floatval($group_additional) );
					}
	...
	function send_user_query_post(){
		if($this->post('action') == 'send_user_query' ){
			...
			if($sender && $subject && $msg && $comp_id){
				...
				if(!empty($eresult) && $eresult->emailid != ''){
					$msg = nl2br($msg);
