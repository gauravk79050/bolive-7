...
class Paypal_lib {
	...
	public function __construct()
	{
		...
		if($this->CI->config->item('paypal_lib_environment') == 'testing')
		$this->paypal_url = 'https://www.sandbox.paypal.com/cgi-bin/webscr';
		else
		$this->paypal_url = 'https://www.paypal.com/cgi-bin/webscr';
		
		$this->ipn_environment = $this->CI->config->item('paypal_lib_environment');
	...
	function paypal_auto_form() 
	{
		...
		$this->CI =& get_instance();
		$this->CI->load->model('Morders');
		$this->CI->load->model('Mgeneral_settings');
		
		$orderdetails = $this->CI->Morders->get_orders($this->fields['item_number']);
		$general_settings = $this->CI->Mgeneral_settings->get_general_settings(array('company_id'=>$orderdetails[0]->company_id));
		
		echo '<html>' . "\n";
		echo '<head><title>Processing Payment...</title></head>' . "\n";
		echo '<body onLoad="document.forms[\'paypal_auto_form\'].submit();">' . "\n";
		echo '<p>'.$general_settings[0]->payment_instructions.'</p>' . "\n";
	...
	function validate_ipn()
	{
		...
		if($this->ipn_environment == 'testing') 
