    ...
    <table cellspacing="2" cellpadding="2" border="0" id="single-order" align="center">		
		<tr>
			<td><table width="100%" border="0" cellspacing="0" cellpadding="2">		
				...
				<tr>
			  		<td align="left"><strong><?php echo _('Order Date')?></strong> : <?php 
					  ...
					  if(!empty($orderData[$j]) && $orderData[$j][0]->phone_reciever != '' && $orderData[$j][0]->option == "2"){
					  	$is_international = true;
					  }
					  //if($_SERVER['REMOTE_ADDR'] == "122.163.238.81"){?>
								<p style="text-align: right">
									<?php /* if(isset($orderData[$j][0]->payment_status)){
          								echo '<span style="color:#6F9300; font-weight:bold; font-size:20px;">'.(($orderData[$j][0]->payment_status != '1')?_('PAID'):_('PENDING')).'</span><br/>';
          							}
                					if($orderData[$j][0]->payment_via_paypal == '2' && isset($orderData[$j][0]->billing_option)){
										// echo $this->lang->line('via_cardgate_pending');
										echo _('via').' '.$orderData[$j][0]->billing_option;
									}elseif($orderData[$j][0]->payment_via_paypal == '1'){
										echo _('via').' '._('Paypal');
				...
				<tr>
					<td width="52%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="2">
						<tr>
							<td bgcolor="#F0F0F0">
								<b><?php echo _('Customer Address')?></b></td>
								<?php if( $is_international ){?><td bgcolor="#F0F0F0"><b><?php echo _('Delivery Address')?></b></td><?php }?>
						</tr>
						<tr>
							<td>
								<strong>
	                        		...
	                        		<br/>
									<?php echo stripslashes($orderData[$j][0]->lastname_c)?>&nbsp;<?php echo stripslashes($orderData[$j][0]->firstname_c)?>
									<br />
									<?php echo stripslashes($orderData[$j][0]->address_c)?>&nbsp; <?php echo stripslashes($orderData[$j][0]->housenumber_c)?>
									<br />
									<?php echo $orderData[$j][0]->postcode_c?>&nbsp;<?php echo stripslashes($orderData[$j][0]->city_c)?>
									<br/>
								...
								<td>
									<strong>
									<?php if(isset($orderData[$j][0]->name)):echo stripslashes($orderData[$j][0]->name);?>&nbsp;<br /><?php endif;?>
								    <?php echo stripslashes($orderData[$j][0]->delivery_streer_address);?>&nbsp;<?php echo stripslashes($orderData[$j][0]->housenumber_c);?><br />
								    <?php echo $orderData[$j][0]->delivery_zip; if($orderData[$j][0]->delivery_city != '0'){?> - <?php echo stripslashes($orderData[$j][0]->delivery_city);}?><br/><?php //echo $order_data['delivery_area'];?>
								    <?php 
								    	if(isset($countries[$orderData[$j][0]->delivery_country])){
											echo $countries[$orderData[$j][0]->delivery_country]->country_name;
								        }	
								     ?><br/>
								     <?php echo _('Mobile');?> :&nbsp;<?php echo $orderData[$j][0]->phone_reciever;?><br />
								     </strong>
								<?php endif;?>
							</td>		
						</tr>	
						...
						<td>
						<br/>
									<!-- <strong> <?php echo _('Telephone No')?>&nbsp;&nbsp;:<?php echo $orderData[$j][0]->phone_c?></strong>
									<br/>-->
									<strong><?php echo _('GSM'); ?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;<?php echo $orderData[$j][0]->phone_c?></strong>
									<br/>
									<strong><?php echo _('Email')?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;<?php echo $orderData[$j][0]->email_c?>&nbsp;-&nbsp;<a class="edit" href="mailto:<?php echo $orderData[$j][0]->email_c?>?subject=Re: <?php echo _('your order')?>  <?php echo $orderData[$j][0]->id?>&amp;body=<?php echo _('Dear client')?>,"><?php echo _('Email')?></a></span></strong>
						</td>
							<td>&nbsp;</td>
						</tr>
						<?php if(isset($orderData[$j][0]->phone_reciever) && $orderData[$j][0]->phone_reciever == ''){?>
						...
						<tr>
							...
							<tr>
								<td width="200" style="padding:3px"><strong><?php echo _('Delivery Date')?></strong></td>
						 		<td width="15%" style="padding:3px"> <?php 
										...
										$day = $orderData[$j][0]->delivery_day;
										$hour= $orderData[$j][0]->delivery_hour;
										$min =$orderData[$j][0]->delivery_minute;
										echo $date_dutch = $day." ".$da." ".$mo_eng." ".$yr." ".(isset($hour) && isset($min)?(_('at')." ".$hour.".".$min._('hr')):'');
								   ?>
								</td>
							...
							<tr>
								...
								<td width="35%" style="padding:3px" colspan="3"><?php echo stripslashes($orderData[$j][0]->delivery_remarks)?></td>
							</tr>
					  		</table></td>
						</tr>
							...
							<td><table width="100%" cellpadding="0" cellspacing="0" border="0" class="override">
								...
								<td style="padding:3px"><?php 
									...
									$day = $orderData[$j][0]->order_pickupday;
									$hour= $orderData[$j][0]->order_pickuptime;
									echo $date_dutch = $day." ".$da." ".$mo_eng." ".$yr." ".(isset($hour)?(_('at')." ".$hour._('hr')):'');
									?>
								</td>
					  		</tr>
							<!-- <tr>
							...
							<tr>
								...
								<td style="padding:3px"><?php echo stripslashes($orderData[$j][0]->order_remarks)?></td>
							</tr>	
							</table></td>
						</tr>	
							...
							<td><table width="100%" cellpadding="0" cellspacing="0" border="0" class="override">
								<tr>
					  			...
					  			<td width="436" style="padding:3px"> <?php   
									$da = date("d",strtotime($orderData[$j][0]->order_pickupdate));
									$mo_eng = date("F",strtotime($orderData[$j][0]->order_pickupdate));
									$yr = date("Y",strtotime($orderData[$j][0]->order_pickupdate));
									
									$hour= $orderData[$j][0]->order_pickuptime;
									echo $date_dutch = $da." ".$mo_eng." ".$yr." ".(isset($hour)?(_('at')." ".$hour._('hr')):'');
								?>
								</td>
							</tr>	
							<!-- <tr>
		...
		<tr>		
			<td>--><table border="1" cellpadding="2" cellspacing="2" width="100%">			
				...
				<?php for($i=0;$i<count($order_details_data[$j]);$i++):?>
				<tr>
					...
					<span style="font-size:11px"><?php echo stripslashes($order_details_data[$j][$i]->pro_remark)?></span><br><br>
				...
				<?php
					...
					if($orderData[$j][0]->disc_client_amount > 0 ){
						$disc_client_amount = $orderData[$j][0]->disc_client_amount;
					}
				?>
				
				<?php if($disc_client_amount > 0 && $orderData[$j][0]->disc_client > 0){?>
				<tr>
					<td colspan="5" align="right" valign="top"><div align="right"><strong><?php echo _('Loyality Discont')?>(<?php echo $orderData[$j][0]->disc_client;?>%) </strong></div></td>
				...
				<?php
					$disc_amount = 0;
					if($orderData[$j][0]->disc_amount > 0 ){
						$disc_amount = $orderData[$j][0]->disc_amount;
					}
				?>
				
				<?php if($disc_amount > 0 && $orderData[$j][0]->disc_percent > 0){?>
				<tr>
					<td colspan="5" align="right" valign="top"><div align="right"><strong><?php echo _('Discount')?>(<?php echo $orderData[$j][0]->disc_percent;?>%) </strong></div></td>
					<td align="right" valign="top"><b>-<?php echo round(( (float)$disc_amount ),2)?>&nbsp;&euro;</b></td>
				</tr>
				<?php }elseif($disc_amount > 0 && $orderData[$j][0]->disc_price > 0){?>
				<tr>
					<td colspan="5" align="right" valign="top"><div align="right"><strong><?php echo _('Discount')?>(<?php echo $orderData[$j][0]->disc_price;?>&euro;) </strong></div></td>
					<td align="right" valign="top"><b>-<?php echo round(( (float)$disc_amount ),2)?>&nbsp;&euro;</b></td>
				</tr>
				<?php }?>
				<!-- ------------------------------------------------------------------ -->
				...
				<tr>
					...
					<td align="right" valign="top"><div align="left"><b>&euro;&nbsp;<span id="total_cost"><?php echo str_replace('.',',',round(( (round(( (float)$total[$j] ),2) + round(( (float)$orderData[$j][0]->delivery_cost ),2)) - (round(( (float)$disc_client_amount ),2)+round(( (float)$disc_amount ),2)) ),2)); ?></span> </b></div></td>
				</tr>
	...
	<table width="550" cellspacing="2" cellpadding="2" id="single-order">		
				...
				<tr>
			  	...
			  	<tr>
					...
					?>
								<?php 
								$is_international = false;
								if(!empty($orderData) && $orderData[0]->phone_reciever != '' && $orderData[0]->option == "2"){
									$is_international = true;
								}
								//if($_SERVER['REMOTE_ADDR'] == "122.163.238.81"){?>
								<p style="text-align: right">
									...
									<?php if($orderData[0]->payment_via_paypal == 2 ){?>
										...
										<?php }?>
										<br/>
										<?php if($orderData['0']->billing_option!=""){ echo _("via")." ".$orderData['0']->billing_option; }?>
									<?php }?>
								</p>
								<?php //}?>
					</td>
				...
				<tr>
					<td width="52%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="override">
						...
						<tr>
							<td>
								<strong>
									<?php echo stripslashes($orderData[0]->lastname_c)?>&nbsp;<?php echo stripslashes($orderData[0]->firstname_c)?>
	                        		<br />
	                        		<?php echo stripslashes($orderData[0]->address_c)?>&nbsp;<?php echo stripslashes($orderData[0]->housenumber_c)?>
	                        		<br />
	                        		<?php echo $orderData[0]->postcode_c?>&nbsp;<?php echo stripslashes($orderData[0]->city_c)?>
	                        		<br/>
							...
							<td>
								    ...
								    <?php 
								    	if(isset($countries[$orderData[0]->delivery_country])){
											echo $countries[$orderData[0]->delivery_country]->country_name;
								        }	
								     ?><br/>
						...
						<td>
						<br/>
                        		<!-- <strong><?php echo _('Telephone')?>&nbsp;&nbsp;:<?php echo $orderData[0]->phone_c?></strong>
                        		<br/>-->
                       			 <strong>GSM &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;<?php if($orderData[0]->phone_c): echo $orderData[0]->phone_c; else: echo _('Not Available');endif;?></strong>
                        		<br/>
                        		<strong><?php echo _('Email')?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp; <?php echo $orderData[0]->email_c?>&nbsp;-&nbsp;<a class="edit" href="mailto:<?php echo $orderData[0]->email_c?>?subject=Re: <?php echo _('your order')?> <?php echo $orderData[0]->id?>&amp;body=<?php echo _('Dear client')?>,"><?php echo _('Email')?></a> </span></strong>
						</td>
							<td>&nbsp;</td>
						</tr>
						
						<?php if(isset($orderData[0]->phone_reciever) && $orderData[0]->phone_reciever == ''){?>
							<?php if($orderData[0]->option == "2"):?>
							<tr>
								<td><table width="100%" cellpadding="0" cellspacing="0" border="0" class="override">
									<tr>
										<td width="20%" style="padding:3px"><strong><?php echo _('Delivery Date')?>: </strong>
										<?php 	
										...
										$hour= $orderData[0]->delivery_hour;
										$min =$orderData[0]->delivery_minute;
										echo $date_dutch = $day." ".$da." ".$mo_eng." ".$yr." ".(isset($hour) && isset($min)?(_('at')." ".$hour.".".$min._('hr')):'');
										?>
										</td>
										<!-- <td width="30%" style="padding:3px"><strong><?php echo _('Delivery Address')?></strong></td>
									...
									<tr>
										<td width="30%" style="padding:3px"><strong><?php echo _('Remarks')?>: </strong><?php echo stripslashes($orderData[0]->delivery_remarks)?></td>
									</tr>
									<tr>
										<td width="30%" style="padding:3px"><strong><?php echo _('Shop')?>: </strong><?php echo $orderData[0]->company_c?></td>
									</tr>	
							</table></td>
							</tr>
							...
							<td><table width="100%" cellpadding="0" cellspacing="0" border="0" class="override">
								...
								$hour= $orderData[0]->order_pickuptime;
								echo $date_dutch = $day." ".$da." ".$mo_eng." ".$yr." ".(isset($hour)?(_('at')." ".$hour._('hr')):'');
								?></td>
							</tr>
							<!-- <tr>
							...
							<tr>
								<td width="20%" style="padding:3px"><strong><?php echo _('Note')?></strong></td>
								<td style="padding:3px"><?php echo stripslashes($orderData[0]->order_remarks)?></td>
							</tr>	
							</table></td>
						</tr>	
		...
		<tr>		
			<td>--><table border="1" cellpadding="2" cellspacing="2" width="100%">			
				...
				<tr>
					...
					<?php if($TempExtracosts != array() && isset($TempExtracosts[$i]) && $TempExtracosts[$i]!= ""):?>
                    	...
                    	<?php for($j=0;$j<count($TempExtracosts[$i]);$j++): ?>
					  		<span style="color:red;"><?php echo $TempExtracosts[$i][$j][0]?></span> : <?php echo $TempExtracosts[$i][$j][2];?>&euro;<br>
						<?php  endfor; ?>
					<?php endif;?>
					...
					<?php if($order_details_data[$i]->pro_remark != ""):?><br>
						<span style="font-size:11px"><?php echo stripslashes($order_details_data[$i]->pro_remark)?></span><br><br>
					<?php else:?>
				...
				<tr>
					<td colspan="5" align="right" valign="top"><div align="right"><strong><?php echo _('Total')?></strong></div></td>	
					<td align="right" valign="top"><b>&euro;&nbsp;<span id="total_cost"><?php echo str_replace('.',',',round(( (round(( (float)$total ),2) + round(( (float)$orderData[0]->delivery_cost ),2)) - (round(( (float)$disc_client_amount ),2)+round(( (float)$disc_amount ),2)) ),2))?></span> </b></td>
				</tr>
				<?php if($activate_discount_card):?>
				<tr>						
