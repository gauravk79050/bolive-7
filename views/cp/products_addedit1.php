...
if(!empty($used_fdd_pro_info)){
	foreach ($used_fdd_pro_info as $fdd_pro_id){
		echo "<div><div class =\"usr_text\"><label><a href=\"javascript:;\" onclick=\"login_at_fdd(".$fdd_pro_id['fdd_pro_id'].")\">".$fdd_pro_id['p_name_dch']."</a></label></div><div class =\"usr_val\">";
   		$str = '';
   		foreach ($product_ingredients as $ing){
   			if(($fdd_pro_id['fdd_pro_id'] == $ing->kp_id) && ($ing->ki_id != 0)){
				$str .= stripslashes($ing->ki_name);
				$str .= (!empty($ing->prefix))?'('.$ing->prefix.')':'';
				$str .= ', ';
		}}
		foreach ($product_ingredients_vetten as $ing){
			if(($fdd_pro_id['fdd_pro_id'] == $ing->kp_id) && ($ing->ki_id != 0)){
				$str .= stripslashes($ing->ki_name).', ';
		}}
		$str = rtrim($str,', ');
		if(!empty($add_ing)){
			$additive_arr = array();
			foreach ($add_ing as $add){
				if(!in_array($add->add_name,$additive_arr) && ($fdd_pro_id['fdd_pro_id'] == $add->kp_id))
					$additive_arr[] = $add->add_name;
			}
			for($i = 0; $i < count($additive_arr); $i++){
				$str .= ", ";
				if($additive_arr[$i] != "others")
					$str .= stripslashes($additive_arr[$i]);
				$count = 0;
				$add_str = "";
				foreach ($add_ing as $add){
					if(($add->add_name == $additive_arr[$i]) && ($fdd_pro_id['fdd_pro_id'] == $add->kp_id) && ($add->ki_name != "")){
						$add_str .= stripslashes($add->ki_name).", ";
						$count = $count+1;
					}
				}
				$add_str = rtrim(trim($add_str),",");
				if($count == 1)
					$str .= " ".$add_str;
				elseif ($count >1){
					if($additive_arr[$i] == "others"){
