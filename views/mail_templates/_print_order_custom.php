<table cellspacing="2" cellpadding="2" border="0" id="single-order" align="center" width="100%">
	<tr>
    	<td>
    		<table width="100%" border="0" cellspacing="0" cellpadding="2">
				<tr>
        			<td align="left">
          				<?php echo _("Order No.");?> : <strong><?php echo $order_id;?></strong>
          			</td>
        		...
        		<tr>
					<td align="left">
          				<?php echo _("Customer");?> : <strong><?php if( $is_international ){echo '';}elseif($client->company_c != '0'){echo $client->company_c.' / ';}else{echo '';}?><?php echo stripslashes($client->firstname_c);?>&nbsp;<?php echo stripslashes($client->lastname_c);?></strong>
          				<br />
						<br />
          			</td>
        		</tr>
			<?php if( !$is_international ) { //IS PICKUP OR NATIONAL DELIVERY ?>
	        	...
	        	<tr>
					<td align="left">
	          			<?php echo _("Pickup Date");?> : <strong><?php echo $order_data['order_pickupday'].' '.date('d/m/Y',strtotime($order_data['order_pickupdate'].' 00:00:00')).' '._('on').' '.$order_data['order_pickuptime'].' '._('hr'); ?></strong>
	          		</td>
	        	</tr>
	        	<tr>
					<td align="left">
	          			<?php echo _("Pickup Note");?> : <strong><?php echo stripslashes($order_data['order_remarks']); ?></strong>
	          		</td>
	        	</tr>
	        	<?php }elseif( $order_data['option'] == '2' ){ //IS NATIONAL DELIVERY ?>
	        	<tr>
					<td align="left">
	          			<?php echo _("Delivery Day");?> : <strong><?php echo $order_data['delivery_day'].' '.date('d/m/Y',strtotime($order_data['delivery_date'].' 00:00:00')).' '.( ($order_data['delivery_hour'] != '' && $order_data['delivery_minute'] != '')?_('on').' '.$order_data['delivery_hour'].':'.$order_data['delivery_minute']._('hr'):'' ); ?></strong>
	          		</td>
	        	</tr>
	        	<tr>
					<td align="left">
	          			<?php echo _("Delivery Note");?> : <strong><?php echo stripslashes($order_data['delivery_remarks']); ?></strong>
	          		</td>
	        	</tr>
	        	<?php }?>
        	<?php }?>
        	<?php if( $is_international ){ ?>
              	<tr>
              		...
              		<?php if(isset($order_data['delivery_country']) && $order_data['delivery_country'] == 21 && isset($order_data['delivery_date']) && $order_data['delivery_date'] != '' && $order_data['delivery_date'] != '0000-00-00'){ ?>
              			<?php echo _('Delivery Day'); ?> : <strong><?php echo date('d/m/Y',strtotime($order_data['delivery_date'].' 00:00:00')); ?></strong>
              		<?php } ?>
    ...
    <tr>
    	<td>
			...
			<?php $disc_per_client_amount = 0; $disc_per_amount = 0; ?>
			...
			<?php if($products_pirceshow_status == '0' ){ ?>
				...
				<tr>
					...
					<td align="right"><?php echo defined_money_format((round($total_cost,2)- (round($disc_per_amount,2)+round($disc_per_client_amount,2)))); ?>&euro;</td>
					<?php } ?>
				</tr>
    ...
    <tr>
    	<tr>
        	<td align="left">
          		<?php echo _("Order Date");?> : <strong><?php echo date('d-m-Y',strtotime($order_data['created_date']));?></strong>
          	</td>
        </tr>
        <tr>
        	<td align="left">
        		IP Address : <strong><?php echo $ip_address;?></strong>
        		<br />
            	<br />
            </td>
        </tr>
	              			...
	              			<?php
	              				if( !$is_international ){ //IS PICKUP OR NATIONAL DELIVERY
							    	if($order_data['get_invoice']){
										echo "<br/>"._("I want an invoice")." : <strong>"._("Yes")."</strong><br/>";
										echo  _("VAT")." : <strong>".$client->vat_c."</strong><br/>";
									}
									if( $order_data['option'] == '1' && $order_data['name'] != '' ){ 
										echo '<br /><strong>'.stripslashes($order_data['name']).'</strong><br />';
									} ?>
								<?php } ?>
								<br />
								<?php echo _("Mobile")?> :&nbsp;<strong><?php echo $client->phone_c;?></strong><br />
								<?php echo _("Email")?> :&nbsp;<strong><?php echo $client->email_c;?></strong><br />
	              		</td>
              		</tr>
				</table>
			</td>	
